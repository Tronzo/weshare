var SocketIp = "kf.wqlcms.com";
var TimeInterval;
var getType = 1;
var ajaxAddress = "http://kf.wqlcms.com/";
...
ChatFloatFactory.prototype = {
    ...
    AjaxRight: function (t, options) {
        ...
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
                ...
                if (obj && obj.wqlcmsid && obj.wqlcmsid.length > 0) {
                    t.initialize(options, obj.wqlcmsid, obj.msgcount, obj.socket);
                    e.setCookie('useright', '{"aid":' + options.AId + ',"wqlcmsid":"' + obj.wqlcmsid + '","socket":' + obj.socket + '}');
                }
            }
        }, function (status) { });
    ...
    SocketMessage: function (AId, wqlcmsid) {
        var e = this;
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
        ...
        ajax.get(ajaxAddress + 'Ajax/CustomerChatAjax.aspx?action=GetFloatSocketMessage&wqlcmsid=' + wqlcmsid + "&AId=" + AId, true);
    },
    StartPolling: function (AId, wqlcmsid) {
        getType = 2;
        clearInterval(TimeInterval);
        TimeInterval = setInterval(GetPolling, 10000);
        this.GetPolling(AId, wqlcmsid);
    },
    GetPolling: function (AId, wqlcmsid) {
        var e = this;
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
        ...
        ajax.get(ajaxAddress + 'Ajax/CustomerChatAjax.aspx?action=GetFloatPolling&wqlcmsid=' + wqlcmsid + "&AId=" + AId, true);
    },
...
window.ChatFloat = function (options) {
    ...
    if (options && options.AId) {
        ...
        if (strStoreData && strStoreData.length > 0) {
            ...
            if (objStorage && objStorage.aid == options.AId && objStorage.wqlcmsid) {
                var socket = 0;
                if (objStorage.socket) {
                    socket = objStorage.socket;
                }
                e.initialize(options, objStorage.wqlcmsid, 0, socket);
                Tools.SocketMessage(options.AId, objStorage.wqlcmsid);
            }
            else {
...
ChatFloat.prototype = {
    initialize: function (options, wqlcmsid, msgcount, socket) {
        var e = this;
        ...
        if (socket == 1) {
            Tools.loadjscssfile("http://" + SocketIp + ":8889/socket.io/socket.io.js", "js", function () { e.socketconnect(options, wqlcmsid); });
        }
    ...
    socketconnect: function (options, wqlcmsid) {
        try {
            ...
            socket.on("connect", function () {
                ...
                socket.on('dataChange', function (json) {
                    if (json) {
                        if (json.msgCount == 1) {
                            Tools.SocketMessage(options.AId, wqlcmsid);
                        }
                    }
                });
            });
