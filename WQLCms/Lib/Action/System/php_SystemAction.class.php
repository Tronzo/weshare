...
class SystemAction extends BackAction
{
  ...
  PRIMARY KEY (`id`)
        ...
        $remoteStr = @wqlcms_getcontents($url);
    ...
    public function doUpdate()
    {
        ...
        if (!isset($_GET['ignore'])) {
            if (!is_writable(($_SERVER['DOCUMENT_ROOT'] . '/WQLCms'))) {
            	$cannotWrite=1;
                $this->error('您的服务器WQLCms文件夹不可写入，设置好再升级', U('System/main'));
            }
        ...
        if (intval($rt['success']) < 1) {
            ...
            file_put_contents($locationZipPath, @wqlcms_getcontents($filename));
...
function wqlcms_getcontents($url){
