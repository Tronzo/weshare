...
class ResearchAction extends LotteryBaseAction
{
    ...
    public function index()
    {
        if ($this->wecha_id&&!$this->fans){
            $this->error('请先完善个人资料再参加活动',U('Userinfo/index',array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'redirect'=>MODULE_NAME.'/index:reid'.intval($this->_rid))));
        }
        $status = 0;
    ...
    public function answer()
    {
        ...
        if (IS_POST) {
            ...
            if (empty($qid) || empty($aids) || empty($this->_rid)) {
                die(json_encode(array('error_code' => true, 'msg' => '不合法的操作')));
            }
            if (empty($qids)) {
