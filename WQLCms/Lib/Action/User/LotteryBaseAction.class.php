...
class LotteryBaseAction extends UserAction
{
                    ...
                    if ($id = $data->add()) {
                        ...
                        if($_POST['researchid']){
                            M('Research')->where(array('id'=>$_POST['researchid']))->save(array('lid'=>$id));
                             $this->success('活动创建成功',U('Research/index', array('token' => session('token'))));
                        }
        ...
        if (IS_POST) {
            ...
            if ($_POST['enddate'] < $_POST['statdate']) {
                ...
                if ($data->where($where)->save($_POST)) {
                    ...
                    $this->success('修改成功', U($activeType . '/index', array(
                        'token' => session('token')
                    )));
                } else {
                    $this->error('操作失败');
    ...
    public function cheat()
    {
        ...
        $records = M('Lottery_cheat')->where(array(
            'lid' => $thisLottery['id']
        ))->order('prizetype asc')->select();
        $this->assign('records', $records);
    }
    public function deleteCheat()
    {
        $record      = M('Lottery_cheat')->where(array(
            'id' => intval($_GET['id'])
        ))->find();
    ...
    public function sn($type)
    {
        ...
        $sendCount   = $Lottery_record_db->where(array(
            'lid' => $id,
            'sendstutas' => 1,
            'islottery' => 1
        ))->count();
    ...
    public function endLottery()
    {
        ...
        $data  = M('Lottery')->where($where)->save(array(
            'status' => 0
        ));
        if ($data != false) {
            M('Users')->where(array(
    ...
    public function del()
    {
        ...
        $type = $this->_get('type');
        if($type=='6'){
            M('Research')->where(array('lid'=>$id))->save(array('lid'=>''));
        }
        if ($back == true) {
     ...
     public function snDelete()
    {
        ...
        switch ($rt['prize']) {
            ...
            case 2:
                M('Lottery')->where(array(
                    'id' => $rt['lid']
                ))->setDec('secondlucknums');
                break;
            case 3:
                M('Lottery')->where(array(
                    'id' => $rt['lid']
                ))->setDec('thirdlucknums');
                break;
            case 4:
                M('Lottery')->where(array(
                    'id' => $rt['lid']
