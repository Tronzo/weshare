...
class TagLibCx extends TagLib {
    ...
    protected $tags   =  array(
        ...
        'wqlcmsif'        =>  array('attr'=>'where','level'=>2),
		...
		'wqlcmslist'    =>  array('attr'=>'array,foreach,offset,length,key,mod','level'=>3,'alias'=>'iterate'),
	...
	public function _wqlcmsif($attr,$content) {
        $tag          = $this->parseXmlAttr($attr,'wqlcmsif');
	...
	public function _wqlcmslist($attr,$content) {
        static $_iterateParseCache = array();
        //如果已经解析过，则直接返回变量值
        $cacheIterateId = md5($attr.$content);
        if(isset($_iterateParseCache[$cacheIterateId]))
            return $_iterateParseCache[$cacheIterateId];
        $tag      = $this->parseXmlAttr($attr,'wqlcmslist');
        $name   = $tag['array'];
        $id        = $tag['foreach'];
        $empty  = isset($tag['empty'])?$tag['empty']:'';
        $key     =   !empty($tag['key'])?$tag['key']:'i';
