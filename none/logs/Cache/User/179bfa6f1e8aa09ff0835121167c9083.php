...
<div class="content">
  ...
  <div class="cLineC" style="line-height: 32px;"> <a href="<?php echo U('Business/poster_add',array('token'=>$token,'type'=>$type));?>" class="btn btn-primary btn_submit  J_ajax_submit_btn">添加宣传海报</a> </div>
...
美容公司名称<?php endif; ?>
          ...
          <tbody>
              <?php if(is_array($busines_pic)): $i = 0; $__LIST__ = $busines_pic;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td>
              <a href="<?php echo U('Business/poster_add',array('pid'=>$vo['pid'],'type'=>$vo['type']));?>" class="btn btn-primary btn_submit  J_ajax_submit_btn">编辑</a>
              <a href="<?php echo U('Business/poster_del',array('pid'=>$vo['pid'],'type'=>$vo['type']));?>" onclick="return(confirm('确定要删除吗？'))" class="btn btn-primary btn_submit  J_ajax_submit_btn">删除</a>
                    </td>
