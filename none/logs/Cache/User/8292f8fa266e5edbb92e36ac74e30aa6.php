...
<div class="content" <?php if(session('isQcloud') == true): ?>style="float:center;"<?php endif; ?>>
  ...
  <a href='<?php echo U("Text/add");?>' title='新增文本自定义回复' class='btn btn-primary btn_submit  J_ajax_submit_btn btn btn-primary btn_submit  J_ajax_submit_btn bigbtn'><img src="<?php echo RES;?>/images/text.png" class="btn btn-primary btn_submit  J_ajax_submit_btn" />新增文本自定义回复</a>　
  <a href='<?php echo U("Img/add");?>' title='新增图文自定义回复' class='btn btn-primary btn_submit  J_ajax_submit_btn btn btn-primary btn_submit  J_ajax_submit_btn bigbtn'><img src="<?php echo RES;?>/images/pic.png" class="btn btn-primary btn_submit  J_ajax_submit_btn" />新增图文自定义回复</a>　
          
            </div>
  <div style="float:right;line-height:30px;">
	<form action="#" method="post">
	<input type="text" placeholder="请输入标题搜索词" value="<?php echo ($_POST['search']); ?>" class="px" name="search" />
	<button class="btn-primary btn_submit  J_ajax_submit_btn"  style="height: 29px;">搜索</button>
	</form>
   </div>
            <div class="clr"></div>
            ...
            <TABLE class="ListProduct" border="0" cellSpacing="0" cellPadding="0" width="100%">
              <THEAD>
                <TR valign="top">
                    ...
                    <TH class="time"><button class="btn btn-primary btn_submit  J_ajax_submit_btn" onclick="$('#sortform').submit()" >排序</button></TH>
              ...
              <TBODY>
				...
				<?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "没有找到数据" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                  <td><div class="answer_text"><img src="<?php echo RES;?>/images/pic.png"  title="图文自定义内容"><?php echo ($vo["title"]); ?></div></td>
