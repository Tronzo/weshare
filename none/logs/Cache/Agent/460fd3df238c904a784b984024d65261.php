...
<div class="columntitle">修改资料</div>
   <form method="post" action="?g=Agent&m=Site&a=index" id="myform">
         ...
         <tr>
          <th>需要审核</th>
          <td><label><input type="radio" name="needcheckuser" value="true" <?php if($thisAgent['needcheckuser']==1)echo checked; ?> /> 注册时无需要审核</label> <label><input type="radio" name="needcheckuser" value="false" <?php if($thisAgent['needcheckuser']!=1)echo checked; ?> /> 注册时需要审核</label> </td>
        </tr>
         <tr style="margin-top:10px;">
          <th>注册填写手机号</th>
          <td><label><input type="radio" name="regneedmp" value="true" <?php if($thisAgent['regneedmp']==1)echo checked; ?> /> 需要填写手机号</label> <label><input type="radio" name="regneedmp" value="false" <?php if($thisAgent['regneedmp']==0)echo checked; ?> /> 不需要填写手机号</label> </td>
        </tr>
         <tr style="margin-top:10px;">
          <th>注册后等级</th>
