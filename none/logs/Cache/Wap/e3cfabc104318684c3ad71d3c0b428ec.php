					...
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><article><!--帖子-->
							...
							<section>
								...
								<?php if($list['photoArr'] != ''): ?><figure data-count="<?php echo (count($list["photoArr"])); ?>张图片">
										<div>
											<?php $__FOR_START_1782641638__=0;$__FOR_END_1782641638__=count($list['photoArr']);for($i=$__FOR_START_1782641638__;$i < $__FOR_END_1782641638__;$i+=1){ ?><img src="<?php echo ($list['photoArr'][$i]); ?>" data-src="<?php echo ($list['photoArr'][$i]); ?>" data-gid="g7" onload="preViewImg(this, event);"/><?php } ?>
										</div>
									</figure><?php endif; ?>
...
<?php if($kefu['lt'] == '1'): ?><a href="<?php echo ($kefu["info2"]); ?>" id="CustomerChatFloat" style="position: fixed; right: 0px; top: 150px; z-index: 99999; height: 70px; width: 65px; min-width: 65px; background-image: url(/tpl/Wap/default/common/css/img/MobileChatFloat.png); background-size: 65px; background-position: 0px 0px; background-repeat: no-repeat no-repeat;"></a><?php endif; ?></body>
