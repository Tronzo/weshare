...
<?php if(is_array($cats)): $i = 0; $__LIST__ = array_slice($cats,0,4,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$hostlist): $mod = ($i % 2 );++$i; if($i%4 == 1): ?><ul><?php endif; ?>

<li><a href="<?php if($hostlist['url'] == ''): echo U('Store/products',array('token' => $_GET['token'], 'catid' => $hostlist['id'], 'wecha_id' => $wecha_id)); else: echo (htmlspecialchars_decode($hostlist["url"])); endif; ?>"><img src="<?php echo ($hostlist["logourl"]); ?>" /><p><?php echo ($hostlist["name"]); ?></p></a></li>
  ...
  <nav class="nav_for_list_ul">
    ...
    <div class="ofh">
      ...
      <ul>
			...
			<?php $__FOR_START_1521957837__=4;$__FOR_END_1521957837__=count($info);for($i=$__FOR_START_1521957837__;$i < $__FOR_END_1521957837__;$i+=1){ ?><li <?php if($i == 4): ?>class="on"<?php endif; ?>></li><?php } ?>
      ...
      <ul>
        ...
        <div id="nav_uls" style="width:33.3%;">
			...
			<ol>
				...
				<?php if(is_array($cats)): $i = 0; $__LIST__ = array_slice($cats,4,null,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$hostlist): $mod = ($i % 2 );++$i;?><li><a href="javascript:swiper_2.slide(<?php echo ($i-1); ?>);" <?php if($i == 1): ?>class="on"<?php endif; ?>><img src="<?php echo ($hostlist["logourl"]); ?>" /><?php echo ($hostlist["name"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
...
this.slide(this.length-3);
  ...
  <div id="list_uls" class="list_uls box_swipe">
	...
	<?php if(is_array($cats)): $i = 0; $__LIST__ = array_slice($cats,4,null,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$hostlist): $mod = ($i % 2 );++$i;?><li>
