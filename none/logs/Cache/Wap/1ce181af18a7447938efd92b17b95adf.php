...
.round li.tel {
background: url(/tpl/Wap/default/common/css/img/tel.png) no-repeat scroll 11px 13px transparent;
...
<div style="float:none;width:100%;" class="headselect">
	<a class="link_tel icon-phone"><?php echo ($info["name"]); ?>详细介绍</a>
</div>
<div class="flexslider">
	<ul class="slides">
   <?php if(is_array($flash[picurl])): foreach($flash[picurl] as $key=>$i): ?><li><img src="<?php echo ($i); ?>"></li><?php endforeach; endif; ?>
	...
	<ul class="round">
		...
		<li>
			...
			<div class="text"><a href=" http://api.map.baidu.com/marker?location=<?php echo ($vo['lat']); ?>,<?php echo ($vo['lng']); ?>&title=<?php echo ($vo['name']); ?>&name=<?php echo ($vo['info']); ?>&content=<?php echo ($vo['address']); ?>&output=html&src=weiba|weiweb"><?php echo ($info["address"]); ?></a></div>
		...
		<li class="tel"><a href="tel:<?php echo ($info["tel"]); ?>">商家电话：<?php echo ($info["tel"]); ?></a></li>
    ...
    <ul class="round">
		...
		<li>
			...
			<div class="text"><?php echo (htmlspecialchars_decode($info["messages"])); ?></div>
...
<footer style="text-align:center; color:#ffd800;margin-right:20px;margin-top:0px;"><a><?php echo ($copyright["copyright"]); ?>——微享技术支持
</a></footer>
...
window.shareData = {  
            ...
            "imgUrl": "<?php echo ($info["banner"]); ?>", 
            "sendFriendLink": "<?php echo ($f_siteUrl); echo U('Fenlei/xiangqing',array('token'=>$_GET['token'], 'pid'=>$_GET['pid'],'id'=>$_GET['id']));?>",
            "tTitle": "<?php echo ($info["name"]); ?>",
            "tContent": "<?php echo (substr($info["info"],0,60)); ?>"
};
</script>
