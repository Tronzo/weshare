...
<!-- <link href="<?php echo RES;?>/css/135/cate25_.css" rel="stylesheet" type="text/css"> -->

<link href="<?php echo RES;?>/css/135/iscroll.css" rel="stylesheet" type="text/css">



<script src="<?php echo RES;?>/css/135/jquery.min.js" type="text/javascript"></script>

<script src="<?php echo RES;?>/css/135/idangerous.swiper.js" type="text/javascript"></script>

<script src="<?php echo RES;?>/css/135/iscroll.js" type="text/javascript"></script>
...
<?php if(is_array($flash)): $i = 0; $__LIST__ = $flash;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$so): $mod = ($i % 2 );++$i;?><li><p><?php echo ($so["info"]); ?></p>
    ...
    <a href="<?php echo ($so["url"]); ?>">
        ...
        <img src="<?php echo ($so["img"]); ?>" style="width: 1366px;">
...
<?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($i < 5): ?><li><a href="<?php if($vo['url'] == ''): echo U('Wap/Index/lists',array('classid'=>$vo['id'],'token'=>$vo['token'])); else: echo (htmlspecialchars_decode($vo["url"])); endif; ?>"><img src="<?php echo ($vo["img"]); ?>"><p><?php echo ($vo["name"]); ?></p></a></li><?php endif; endforeach; endif; else: echo "" ;endif; ?>           
...
<?php if(is_array($info)): $tt = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($tt % 2 );++$tt; if($tt > 4): ?><div class="catemenu " >
...
<?php if ($vo['sub']){ foreach ($vo['sub'] as $res){ if ($res['url']==''){ $url=U('Wap/Index/lists',array('classid'=>$res['id'],'token'=>$res['token'])); }else { $url=htmlspecialchars_decode($res['url']); } echo '<a href="'.$url.'">'.$res['name'].'</a>'; } } ?>
