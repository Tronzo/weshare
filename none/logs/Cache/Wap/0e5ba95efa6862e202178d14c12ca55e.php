...
<ul id="indicator">
				...
				<?php if(is_array($flash[picurl])): foreach($flash[picurl] as $key=>$i): ?><li class="active"></li><?php endforeach; endif; ?>
...
<ul class="mainmenu">
	...
	<?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
			...
			<a href="<?php if($vo['url'] == ''): echo U('Wap/Fenlei/info',array('id'=>$vo['id'],'token'=>$vo['token'])); else: echo (htmlspecialchars_decode($vo["url"])); endif; ?>" >

				<img src="<?php echo ($vo["topic"]); ?>" />

				<p><?php echo ($vo["title"]); ?></p><p><?php echo ($vo["info"]); ?></p>
...
<?php echo ($copyright["copyright"]); ?>——微乾隆技术支持
...
window.shareData = {  
            ...
            "imgUrl": "<?php echo ($copyright["tp"]); ?>", 
            "sendFriendLink": "<?php echo ($f_siteUrl); echo U('Fenlei/index',array('token'=>$info['token']));?>",
            "tTitle": "<?php echo ($copyright["title"]); ?>",
            "tContent": "<?php echo (substr($copyright["info"],0,60)); ?>"
};
</script>
